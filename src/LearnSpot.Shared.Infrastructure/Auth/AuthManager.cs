...
using JsonWebToken = LearnSpot.Shared.Abstractions.Auth.JsonWebToken;
...
internal sealed class AuthManager : IAuthManager
{
    ...
    public JsonWebToken CreateToken(string userId, string role = null, string audience = null, IDictionary<string, IEnumerable<string>> claims = null)
    {
        ...
        {
            throw new ArgumentException("User ID claim (sub) cannot be empty", nameof(userId));
        }
        ...
        var jwtClaims = new List<Claim>
        {
            ...
            new(JwtRegisteredClaimNames.Iat,
                new DateTimeOffset(now).ToUnixTimeMilliseconds().ToString()),
        };
        ...
        var expires = now.Add(_options.Expiry.Value);
